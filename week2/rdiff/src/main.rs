...
fn lcs(seq1: &Vec<String>, seq2: &Vec<String>) -> Grid {
    ...
    let mut lcs_table = Grid::new(seq1.len() + 1, seq2.len() + 1);
    for i in 0..lcs_table.size().0 {
        lcs_table.set(i, 0, 0);
    }
    for j in 0..lcs_table.size().1 {
        lcs_table.set(0, j, 0);
    }
    for i in 1..lcs_table.size().0 {
        for j in 1..lcs_table.size().1 {
...
fn main() {
    ...
    let lines1 = read_file_lines(filename1).expect("The first filename is invalid.");
    let lines2 = read_file_lines(filename2).expect("The second filename is invalid.");
